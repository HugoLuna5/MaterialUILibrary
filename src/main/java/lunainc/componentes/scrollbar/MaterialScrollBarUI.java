package lunainc.componentes.scrollbar;
...
public class MaterialScrollBarUI extends BasicScrollBarUI {
	...
	private void setIconArrowButton(JButton button, int orientation) {
		...
		if (orientation == SwingConstants.NORTH){
			button.setIcon(new ImageIcon(ImagenesMaterial.UP_ARROW));
			return;
		}else if(orientation == SwingConstants.SOUTH){
			button.setIcon(new ImageIcon(ImagenesMaterial.DOWN_ARROW));
			return;
		}else if(orientation == SwingConstants.EAST){
			button.setIcon(new ImageIcon(ImagenesMaterial.RIGHT_ARROW));
			return;
		}else if(orientation == SwingConstants.WEST){
			button.setIcon(new ImageIcon(ImagenesMaterial.LEFT_ARROW));
			return;
		}
	...
	protected JButton installButton(int orientation){
		...
		if(UIManager.getBoolean("ScrollBar[MouseHover].enable")){
			button.addMouseListener(MaterialUIMovimiento.getStaticMovement(button,UIManager.getColor("ScrollBar[MouseHover].color"),
